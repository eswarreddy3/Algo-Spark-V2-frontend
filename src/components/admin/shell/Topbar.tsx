...
import { AlertTriangle, Bell, Check, CheckCircle2, ChevronDown, Info, LogOut, Menu, Moon, Search, Settings, Sun, Users, X } from "lucide-react";
import { C, FB, FD, FM, blueGrad, tint } from "../theme";
import { ALERTS, searchAdmin, type AdminSearchResult, type AdminTarget } from "../data/search";
...
const KIND_COLOR: Record<AdminSearchResult["kind"], string> = {
  Student: C.royal,
  Lab: C.cyan,
  Exam: C.goldDeep,
  Ticket: C.red,
...
const TONE_ICON = { info: Info, warn: AlertTriangle, good: CheckCircle2 };
const TONE_COLOR = { info: C.blue, warn: C.goldDeep, good: C.green };
...
export function Topbar({ onOpenMenu }: { onOpenMenu: () => void }) {
  ...
  const openTarget = useCallback(
    (target: AdminTarget) => {
      if (target.type === "student") nav.openStudent(target.studentId);
      else if (target.type === "lab") nav.openLab(target.labId);
      else if (target.type === "exam") nav.openExam(target.examId);
      else if (target.type === "ticket") nav.openTicket(target.ticketId);
      else if (target.type === "authoring") nav.openAuthoring();
  ...
  return (
    ...
    >
      ...
      <div ref={searchRef} className="ad-search" style={{ position: "relative", flex: 1, minWidth: 0, maxWidth: 440 }}>
        <label
          style={{
        ...
        >
          ...
          <input
            ...
            placeholder="Search students, labs, exams, tickets…"
          ...
          {query ? (
            ...
            <span className="ad-hide-sm" style={{ fontFamily: FM, fontSize: 11, color: C.inkMute, border: `1px solid ${C.line}`, borderRadius: 6, padding: "2px 6px" }}>⌘K</span>
          )}
        </label>
          ...
          >
            {results.length === 0 ? (
              ...
              results.map((r, i) => (
                ...
                >
                  <span style={{ fontFamily: FM, fontSize: 10, color: KIND_COLOR[r.kind], background: tint(KIND_COLOR[r.kind], 12), borderRadius: 6, padding: "3px 7px", flex: "none", width: 66, textAlign: "center" }}>
                    {r.kind}
                  </span>
      ...
      <button
        ...
        className="ad-hide-md"
        style={{ display: "flex", alignItems: "center", gap: 7, background: tint(C.royal, 12), color: C.royal, padding: "7px 12px", borderRadius: 999, fontWeight: 600, fontSize: 13.5, border: "none", cursor: "pointer", fontFamily: FB }}
      >
        <Users size={15} /> {COHORT.activeToday} active today
      </button>
      ...
      <div ref={panelRef} style={{ position: "relative", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        {panel === "bell" && (
          ...
          >
            ...
            <div style={{ maxHeight: 360, overflowY: "auto" }}>
              {ALERTS.map((a, i) => {
                const isUnread = a.unread && !read.includes(a.id);
                const Icon = TONE_ICON[a.tone];
                return (
                  <button
                    key={a.id}
                    onClick={() => { setRead((r) => [...r, a.id]); openTarget(a.target); }}
                    className="ad-row"
                    style={{ width: "100%", textAlign: "left", border: "none", background: isUnread ? tint(C.blue, 6) : C.white, cursor: "pointer", padding: "12px 14px", borderTop: i ? `1px solid ${C.line}` : "none", display: "flex", gap: 11 }}
                  >
                    <span style={{ width: 26, height: 26, flex: "none", borderRadius: 8, background: tint(TONE_COLOR[a.tone], 12), color: TONE_COLOR[a.tone], display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <Icon size={14} />
                    </span>
                    <span style={{ flex: 1, minWidth: 0 }}>
                      <span style={{ display: "block", fontSize: 14, fontWeight: 600, color: C.ink }}>{a.title}</span>
                      <span style={{ display: "block", fontSize: 13, color: C.inkSoft, marginTop: 2, lineHeight: 1.45 }}>{a.body}</span>
                      <span style={{ display: "block", fontFamily: FM, fontSize: 11, color: C.inkMute, marginTop: 5 }}>{a.when}</span>
                    </span>
                  </button>
                );
              })}
        ...
        {panel === "account" && (
          <div
            className="ad-pop"
            style={{ position: "absolute", top: "calc(100% + 10px)", right: 0, width: 250, background: C.white, border: `1px solid ${C.line}`, borderRadius: 14, boxShadow: `0 18px 44px ${C.shadow}`, overflow: "hidden", zIndex: 50 }}
          >
            <div style={{ padding: "13px 14px", borderBottom: `1px solid ${C.line}` }}>
              <div style={{ fontFamily: FD, fontWeight: 600, fontSize: 14.5 }}>{session?.name ?? "Dr. Latha Nair"}</div>
              <div style={{ color: C.inkMute, fontSize: 12.5, marginTop: 2 }}>Training &amp; Placement Officer</div>
              <div style={{ color: C.inkMute, fontSize: 12.5, marginTop: 2 }}>{COHORT.college}</div>
            </div>
            {[
              { label: "Reports", icon: Settings, action: () => nav.go("reports") },
              { label: "Support queue", icon: Bell, action: () => nav.go("support") },
            ].map((item) => (
              <button
                key={item.label}
            ...
            <button
              onClick={() => { setPanel("none"); signOut(); router.replace("/login?role=admin"); }}
              className="ad-row"
